...
function index(
    ...
    url_datatables = base_url + "Transaksi/get_datatables";
    url_add = base_url + "Transaksi/add";
    url_saldo = base_url + "Transaksi/get_saldo";
    url_data = base_url + "Transaksi/get_transaksi";
...
function open_lihat_modal(id){
    $.ajax({
        ...
        success: function (res) {
            ...
            $("input[name='see-uraian'").val(res.data[0].uraian);
...
function open_tambah_modal() {
    ...
    var jenistransaksi  = $("input[name='add-jenistransaksi']").val();
    if(page == "pengeluaran" && jenistransaksi != "Adjustment Bank"){
        init_tambah_modal();
    }else{
        $('#add-form')[0].reset();
        $('#modal-tambah').modal({ backdrop: 'static', keyboard: false });
